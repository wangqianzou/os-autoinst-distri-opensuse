...
sub virsh_migrate_manual_postcopy {
    ...
    enter_cmd("clear");
    $_ret = script_run("(set -x;unset migrate_postcopy;export migrate_postcopy=1; for i in `seq 87000`;do migrate_postcopy=\$?;$_command[1]; if [ \$migrate_postcopy -eq 0 ];then set +x;break;fi; done; if [ \$migrate_postcopy -eq 0 ];then echo -e \"Migrate Postcopy Succeeded! \\n\";else echo -e \"Command not found! \\n\";fi)", timeout => 300);
